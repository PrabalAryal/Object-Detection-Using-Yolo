...
def process_video(input_path, output_path):
    ...
    while cap.isOpened():
        ...
        output = cv2.dnn.NMSBoxes(
            boxes_np.tolist(), confidences_np.tolist(), 0.25, 0.45
        )
        if len(output) > 0:
        ...
        for ind in index:
            ...
            cv2.putText(
                frame, text, (x, y - 10), cv2.FONT_HERSHEY_PLAIN, 0.7, (0, 0, 0), 1
            )
...
def process_video_endpoint():
    ...
    output_filename = os.path.join(
        PROCESSED_VIDEO_DIR, f"processed_{os.path.basename(input_path)}"
    )
